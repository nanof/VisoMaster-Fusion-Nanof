...
SWAPPER_LAYOUT_DATA: Any = {  # noqa: F811
    "Swapper": {
        ...
        "SequentialTargetMatchEnableToggle": {
            ...
            "default": False,
            "help": "No Find Faces required. Uses only checked Input Faces in list order: detection 0 gets input 0, detection 1 gets input 1, and if there are more detections than inputs, wraps to input 0 again (round-robin). Cosine similarity is not used. Assignments are stabilized across frames: ByteTrack IDs when Face Tracking is on and IDs are valid; otherwise IoU and, when overlap is low, nearest screen position vs remembered slots (each slot expires after ~90 frames of not seeing that face) so one person leaving frame briefly does not reshuffle inputs for the others. Frames with zero faces still advance this memory (no false reset from gaps). Large timeline jumps or backward seeks clear state. Swap/restorer/mask settings use the same parameter set as normal swap: the selected Find Faces card if any, otherwise the current face-parameter panel (current_widget_parameters). Recognition uses the active swapper's ArcFace model for the target embedding. Ignored when 'Swap Input Face only once' is enabled (Settings → Swap settings). Use 'Input rotate start offset' (or window keys comma / full stop) to shift which checked input is treated as index 0 for the first detection.",
        },
