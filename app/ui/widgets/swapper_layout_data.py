...
SWAPPER_LAYOUT_DATA: Any = {  # noqa: F811
    "Swapper": {
        ...
        "SecondarySwapperEnableToggle": {
            ...
            "help": "Run a second swapper and blend it with the primary. Hidden when the primary model cannot mix (Ghost, DFM, SimSwap512, CSCS). Off by default; Blend 0 skips the extra pass.",
        },
        "SecondarySwapModelSelection": {
            ...
            "help": "Second model to blend. Inswapper-family models share w600k ArcFace. HyperSwap needs Allow HyperSwap mix. BlendSwap/UniFace/ReHiFace-S are available here even when they are not the primary picker.",
        },
        "SecondarySwapperResSelection": {
            ...
            "help": "Rendering resolution for Inswapper/AlphaFace/InStyle secondary passes. Other models keep their native size.",
        },
        "SecondarySwapperBlendModeSelection": {
            "level": 2,
            "label": "Blend Mode",
            "options": ["Linear", "Identity / Detail", "Center weighted"],
            ...
            "help": "Linear = RGB lerp. Identity / Detail keeps primary shape (lows) and pulls texture from the secondary (highs). Center weighted mixes more secondary in the face interior.",
        },
        "SecondarySwapperBlendAmountSlider": {
            ...
            "help": "0 = primary only (no extra inference). 100 = full secondary contribution. In paused preview, moving this remixes cached crops without re-running the networks.",
        },
        "SecondarySwapperHyperSwapMixEnableToggle": {
            "level": 2,
            "label": "Allow HyperSwap mix",
            "default": False,
            "parentToggle": "SecondarySwapperEnableToggle",
            "requiredToggleValue": True,
            "parentSelection": "SwapModelSelection",
            ...
            "help": "Required when either model is HyperSwap. Alignments differ from Inswapper, so some ghosting is expected. Off skips the secondary pass for HyperSwap pairs.",
        },
        ...
        "RandomTargetMatchEnableToggle": {
            ...
            "help": "Same pipeline as 'Swap all by index' (no Find Faces, checked Input Faces only, no cosine matching), but each new physical face is assigned a random checked input that is not already used by another face in the frame (no repeats while enough inputs remain; if there are more detections than checked inputs, the biggest faces keep the distinct inputs and the smallest ones reuse). Existing faces keep their assignment while tracking/stabilization can match them — including across pause/play and capture reopen. Large timeline jumps or backward seeks still clear sticky state. Press X to reshuffle all random assignments. Ctrl+Alt+Left-click an Input Face card (or use its context menu) to mark it fixed so X does not reassign it. Mutually exclusive with 'Swap all by index'. Ignored when 'Swap Input Face only once' is enabled.",
        },
    ...
    "Swap strength and likeness": {
        ...
        "StrengthAmountSlider": {
            ...
            "parentToggle": "StrengthEnableToggle",
            "requiredToggleValue": True,
            "help": "Iterations for the primary model (100 = 1 pass). 0 turns the primary swap off but still runs the rest of the pipeline.",
        },
        "SecondaryStrengthAmountSlider": {
            ...
            "requiredToggleValue": True,
            "help": "Iterations for the secondary model (100 = 1 pass). 0 turns the secondary swap off.",
        },
