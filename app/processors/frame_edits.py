...
from app.processors.face_editors import (
    expression_lp_stitch_profile_scope,
    slice_lp_motion_batch,
)
...
def _lp_motion_batch2_disabled() -> bool:
    return os.environ.get("VISIOMASTER_LP_MOTION_NO_BATCH2", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _face_expression_has_active_work(parameters: dict, mode: str) -> bool:
    """
    False when the restorer would accumulate zero motion (e.g. Simple with neither eyes nor lips).
    Avoids motion/appearance/warp cost for no-op configs.
    """
    if mode == "Simple":
        animation_region = (
            parameters.get("FaceExpressionAnimationRegionSelection", "all") or "all"
        )
        if not isinstance(animation_region, str):
            return True
        return (
            "all" in animation_region
            or "eyes" in animation_region
            or "lips" in animation_region
        )
    perf = parameters.get(
        "FaceExpressionAdvancedPerformanceEyesLipsOnlyToggle", False
    )
    if parameters.get("FaceExpressionEyesToggle", False):
        return True
    if parameters.get("FaceExpressionLipsToggle", False):
        return True
    if not perf and parameters.get("FaceExpressionBrowsToggle", False):
        return True
    if not perf and parameters.get("FaceExpressionGeneralToggle", False):
        return True
    return False
...
class FrameEdits:
    ...
    def apply_face_expression_restorer(
        ...
        with stream_context:
            with expression_lp_stitch_profile_scope(self.models_processor):
                # --- CONFIGURATION ---
                ...
                # Motion: one batched run (driving + target) when ORT/TRT accepts batch 2.
                if not _lp_motion_batch2_disabled():
                    try:
                        motion_b = self.models_processor.lp_motion_extractor(
                            torch.stack(
                                [driving_face_256.float(), target_face_256.float()],
                                dim=0,
                            ),
                            "Human-Face",
                        )
                        x_d_i_info = slice_lp_motion_batch(motion_b, 0)
                        x_s_info = slice_lp_motion_batch(motion_b, 1)
                    except Exception:
                        _logger.debug(
                            "LivePortrait motion batch-2 failed; using two sequential runs.",
                            exc_info=True,
                        )
                ...
                if mode == "Simple":
                    ...
                    if parameters.get(
                        ...
                        flag_activate_brows = False
                        flag_activate_general = False
                ...
                eps = 0.0
                raw_eps = os.environ.get("VISIOMASTER_EXPR_SKIP_WARP_EPS", "").strip()
                if raw_eps and not _env_truthy_local("VISIOMASTER_EXPR_NO_SKIP_WARP"):
                    try:
                        eps = float(raw_eps)
                    except ValueError:
                        eps = 0.0
                ...
                skip_neural_decode = (
                    eps > 0.0
                    and float(torch.max(torch.abs(accumulated_motion)).item()) <= eps
                )

                if skip_neural_decode:
                    # Saves appearance extractor + warp when motion is effectively zero.
                    out = target_face_512.float().mul_(1.0 / 255.0).clamp_(0, 1)
                    if out.dim() == 4:
                        out = out.squeeze(0)
                else:
    ...
    def apply_perform_recast(
        ...
        with stream_context, torch.inference_mode():
            ...
            # the shared slider so existing sessions keep working.
            ...
            # Detection bboxes derived from actual tensor size (not hardcoded
            # to 512). In VR180 mode the face tensor is not guaranteed to be
            # exactly 512×512, and a mismatched bbox produced bad landmarks.
            ...
            driving_face_512, _, _ = faceutil.warp_face_by_face_landmark_x(
                ...
                # extends past the image (VR180 black-crop bug).
                padding_mode="border",
            )
            driving_face_256 = self.t256_face(driving_face_512)
            ...
            # The generator can emit an all-black (or NaN/Inf) frame when it is
            # fed an out-of-distribution crop (too-tight scale, extreme pose in
            # fp16). Rather than paste a black face into the result, fall back to
            # the un-recast swap for this frame.
            ...
            # the surrounding swap instead of showing a hard 512-crop seam.
        ...
        # Sync the stream safely (mirrors apply_face_expression_restorer).
        if local_stream:
            local_stream.synchronize()
