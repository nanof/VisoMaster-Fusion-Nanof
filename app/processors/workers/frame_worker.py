...
class _GatherPrePrimaryRestorer(BaseException):
    """swap_core signals PERF-008 gather: primary ORT batch will run before paste."""
...
class FrameWorker(threading.Thread):
    ...
    def __init__(
        ...
        # Feeder-ordered Swap-all-by-index indices (detection index → input slot)
        self.precomputed_rr_input_indices: list[int] | None = None
                ...
                try:
                    ...
                    if len(task) >= 12:
                        (
                            ...
                            self.precomputed_rr_input_indices,
                        ) = task[:12]
                    elif len(task) >= 11:
                        (
                            self.frame_number,
                        ...
                        self.precomputed_rr_input_indices = None
                    elif len(task) >= 10:
                        (
                            self.frame_number,
                            self.frame,
    ...
    def process_and_emit_task(self):
        ...
        try:
            ...
            # Single-frame mode has its own error recovery path.
            if (
                ...
                try:
                    ...
                    if self.video_processor.file_type in ("webcam", "screen"):
                        self.video_processor.webcam_frame_processed_signal.emit(
                            fallback_bgr
                        )
                    else:
    ...
    def _musetalk_apply_bgr(
        ...
        *,
        ...
        try:
            ...
            use_bboxes = bboxes if bboxes is not None else self._musetalk_bboxes()
    ...
    def _musetalk_apply_vr_crop(
        ...
        *,
        ...
        try:
            ...
            bbox = np.array([pad, pad, w - pad, h - pad], dtype=np.float32)
            dense = None
    ...
    def _project_crop_bbox_to_equirect(
        ...
        R1 = FrameWorker._rodrigues_np(z_axis * math.radians(rot_theta))
    ...
    def _detect_faces_vr_tiled(
        ...
        Catches faces that are invisible to the frame-domain detector because
        of projection distortion (high elevation, near an eye's edge, or very
    ...
    def _process_frame_vr180(
        ...
        processed_perspective_crops_details = []  # each entry: (is_left_eye, tensor, theta, phi, fov)
    ...
    def _primary_restorer_batch_param_tuple(
        self, parameters: dict, control: dict
    ) -> tuple:
        return (
            bool(parameters.get("FaceRestorerEnableToggle", False)),
            str(parameters.get("FaceRestorerDetTypeSelection", "")),
            str(parameters.get("FaceRestorerTypeSelection", "")),
            float(parameters.get("FaceFidelityWeightDecimalSlider", 0.9)),
            int(parameters.get("FaceRestorerBlendSlider", 100)),
    ...
    def _restorer_infer_cache_key(
        ...
        return (
            ...
            bool(parameters.get("SecondarySwapperEnableToggle", False)),
    ...
    def _plane_ordered_share_primary_restorer_batch_key(
        ...
        key0 = self._primary_restorer_batch_param_tuple(
            ordered[0]["parameters"], ordered[0]["control"]
        )
        t0 = self.get_face_similarity_tform(sm, ordered[0]["kps_5"])
        if not ordered[0]["parameters"].get("FaceRestorerEnableToggle", False):
            return False
        if self._face_restorer_skip_for_small_face(
            ordered[0]["parameters"], float(t0.scale)
        ):
            return False
        rt0 = self._face_restorer_effective_type(
            ordered[0]["parameters"],
            ordered[0]["parameters"]["FaceRestorerTypeSelection"],
            float(t0.scale),
            is_live_stream=live,
        )
        for s in ordered[1:]:
            p = s["parameters"]
            c = s["control"]
            if p["SwapModelSelection"] != sm:
                return False
            if self._primary_restorer_batch_param_tuple(p, c) != key0:
                return False
            if not p.get("FaceRestorerEnableToggle", False):
                return False
            tf = self.get_face_similarity_tform(sm, s["kps_5"])
            if self._face_restorer_skip_for_small_face(p, float(tf.scale)):
                return False
            rt = self._face_restorer_effective_type(
                p,
                p["FaceRestorerTypeSelection"],
                float(tf.scale),
                is_live_stream=live,
            )
            if rt != rt0:
    ...
    def _gather_primary_restorer_batch_eligible(
        self,
        parameters: dict,
        ...
        if self._face_restorer_skip_for_small_face(parameters, tform_scale):
            return False
        if not parameters.get("FaceRestorerEnableToggle", False):
            return False
        if parameters.get("FaceRestorerDetTypeSelection") != "Original":
            return False
    ...
    def _plane_swap_prefetched_apply_swap_core_sequence(
        ...
        use_gather = (
            ...
            and self._plane_ordered_share_primary_restorer_batch_key(ordered)
            and self.models_processor.face_restorers.restorer_ort_batched_attempt_enabled()
        )
        ...
        if gather_tails is not None:
            for _idx, (s, pre) in enumerate(zip(ordered, prefetched_list)):
                try:
                    ...
                    continue
            bo: torch.Tensor | None = None
            if len(gather_tails) == len(ordered):
                p0 = ordered[0]["parameters"]
                sm0 = p0["SwapModelSelection"]
                fdw = float(p0["FaceFidelityWeightDecimalSlider"])
                tf0 = self.get_face_similarity_tform(sm0, ordered[0]["kps_5"])
                rt0 = self._face_restorer_effective_type(
                    p0,
                    p0["FaceRestorerTypeSelection"],
                    float(tf0.scale),
            ...
            if bo is not None and bo.shape[0] == len(ordered):
                out_img = img
                for s, pre, entry, row in zip(
                    ordered, prefetched_list, gather_tails, bo.unbind(0)
                ):
                    _pre_swap, _swap_orig, fn = entry
                    out_img, s["fface"]["original_face"], s["fface"]["swap_mask"] = fn(
                        row.contiguous()
                    )
    ...
    def _run_inswapper128_batched_plane_swap_all(
        ...
        for s in ordered:
            ...
            original_faces = self.get_transformed_and_scaled_faces(
                ...
                batch_snap,
                interp_mode=_interp,
                only_256=self._alignment_only_256(swap_model, params),
            )
            _sl = None
    ...
    def _run_ort_256_swapper_batched_plane_swap_all(
        ...
        for s in ordered:
            ...
            original_faces = self.get_transformed_and_scaled_faces(
                ...
                batch_snap,
                interp_mode=_interp,
                only_256=self._alignment_only_256(swapper_model, params),
            )
            _sl = None
            if source_latent_cache is not None and s["s_e"] is not None:
    ...
    def _process_frame_standard(
        ...
        if (
            ...
            _rr_pre = self.precomputed_rr_input_indices
            for _w in _work_faces:
                _entry: dict[str, Any] = {
                    "kps_5": _w["kps_5"],
                    "kps_all": _w["kps_all"],
                ...
                if (
                    _rr_pre is not None
                    and _det_i < len(_rr_pre)
                    and int(_rr_pre[_det_i]) >= 0
                ):
                    _entry["_rr_input_idx"] = int(_rr_pre[_det_i])
                det_faces_data_for_display.append(_entry)
        ...
        if _sequential_match_active and _checked_inputs_ordered:
            ...
            if not _need_rr_apply:
                for _ff in det_faces_data_for_display:
                    if _ff.get("_rr_input_idx") is None:
                        _need_rr_apply = True
                        break
            if _need_rr_apply:
    ...
    def _secondary_swapper_runtime_parameters(
        ...
        """Copy per-face params with secondary model/resolution/strength mapped in."""
        sec_parameters = copy_mapping_data(parameters)
        sec_res_choice = str(parameters.get("SecondarySwapperResSelection", "Auto"))
        sec_parameters["SwapModelSelection"] = sec_model
        sec_parameters["SwapperResSelection"] = sec_res_choice
        sec_parameters["AlphaFaceResSelection"] = sec_res_choice

        use_instyle_512 = sec_res_choice == "512"
        if sec_res_choice == "Auto" and tform is not None:
            try:
                use_instyle_512 = float(tform.scale) <= 1.10
            except (TypeError, AttributeError, ValueError):
                use_instyle_512 = False
        sec_parameters["InStyleResAEnableToggle"] = use_instyle_512
        sec_parameters["InStyleResBEnableToggle"] = use_instyle_512
        sec_parameters["InStyleResCEnableToggle"] = use_instyle_512

        strength_enabled = bool(parameters.get("StrengthEnableToggle", False))
        if strength_enabled:
            sec_amount = float(parameters.get("SecondaryStrengthAmountSlider", 100))
        else:
            sec_amount = 100.0
    ...
    def swap_core(
        ...
        _only_256 = self._alignment_only_256(swapper_model, parameters)
        ...
        if parameters["FaceRestorerEnableToggle"]:
            ...
            if self._face_restorer_skip_for_small_face(parameters, float(tform.scale)):
                ...
                return _swap_core_after_primary_restorer(swap_restorecalc)
            ...
            if (
                gather_pre_primary_restorer_tails is not None
                and self._gather_primary_restorer_batch_eligible(
                    parameters, _rt1, control, float(tform.scale)
                )
            ):
                gather_pre_primary_restorer_tails.append(
                    (
                        swap.clone(),
                        swap_original,
                        _swap_core_after_primary_restorer,
                    )
                )
