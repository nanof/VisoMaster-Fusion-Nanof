...
def _trt_profiles_from_onnx_inputs(
    ...
    """Build ``trt_profile_*`` for every graph **input** (excluding initializers).

    ORT TensorRT EP requires **all** dynamic-rank inputs to appear in the profile
    strings; a single wrong or missing name triggers
    ``User needs to provide all the dynamic shape inputs with associated profiles``.

    Supported pattern per input:

    - Fully static shape → same min/opt/max.
    - Rank ≥ 1 with **only** dimension 0 symbolic / zero (batch) and remaining dims
      known positive integers → min ``1x…``, opt ``batch_opt x …``, max ``batch_max x …``.

    Any other dynamic pattern returns ``None`` (caller skips TRT profiles for that model).
    """
    try:
...
def _tensorrt_dynamic_shape_profile_opts(
    model_name: str, onnx_path: str | None = None
) -> dict[str, str] | None:
    """Return ``trt_profile_{min,opt,max}_shapes`` for ORT TensorRT EP when this model uses variable batch.

    Merges into per-session ``model_trt_options`` in ``load_model``. Shapes follow the
    tensor names and ranks used in ``face_editors`` / ``face_swappers`` I/O binding.

    ``Inswapper128ArcFace`` profiles are derived from the **ONNX file** (real input
    names and ranks) so every dynamic input is listed — the previous hard-coded
    ``input:…`` string failed on graphs whose first tensor is not named ``input``.

    Opt-out / tuning (all optional):

    - ``VISIOMASTER_TRT_NO_DYNAMIC_PROFILES=1`` — disable every profile below (plain TRT build).
    - ``VISIOMASTER_LP_MOTION_TRT_STATIC_BATCH=1`` — **legacy**: omit profile **only** for
      ``LivePortraitMotionExtractor`` (batch-1-only engine cache; same as before this refactor).
    - ``VISIOMASTER_TRT_MAX_BATCH_SWAP`` (default 16), ``VISIOMASTER_TRT_OPT_BATCH_SWAP`` (default 4)
      — Inswapper128 + GhostFace + HyperSwap batched paths.
    - ``VISIOMASTER_TRT_MAX_BATCH_LP_MOTION`` (default 8), ``VISIOMASTER_TRT_OPT_BATCH_LP_MOTION`` (default 2)
      — ``LivePortraitMotionExtractor`` ``img`` (matches Python cap ``bs <= 8``).
    - ``VISIOMASTER_TRT_MAX_BATCH_LP_STITCH`` (default 12), ``VISIOMASTER_TRT_OPT_BATCH_LP_STITCH`` (default 4)
      — LivePortrait stitching / eye / lip ``input`` rows (concat keypoint features).
    - ``VISIOMASTER_TRT_MAX_BATCH_ARCFACE`` (default 16), ``VISIOMASTER_TRT_OPT_BATCH_ARCFACE`` (default 8)
      — batch axis (dim 0) for ArcFace ONNX inputs when profiles are inferred from file.
    - ``VISIOMASTER_TRT_SKIP_ARCFACE_ONNX_PROFILE=1`` — do not attach TRT profiles for
      ``Inswapper128ArcFace`` (use if ``onnx`` is unavailable or the graph uses unsupported dynamics).

    If engine build fails for a model (wrong input name or rank), set
    ``VISIOMASTER_TRT_NO_DYNAMIC_PROFILES=1`` or narrow the affected ``*_MAX_BATCH_*`` env.
    """
    ...
    # ArcFace: must list every dynamic input with correct ONNX names (from file).
    ...
    # (min_shapes, opt_shapes, max_shapes) — comma joins multiple inputs on one line.
...
def _merge_tensorrt_dynamic_shape_profiles(
    model_name: str, model_trt_options: dict, onnx_path: str | None = None
) -> dict:
    """Attach TRT min/opt/max shape profiles when this model has a registry entry."""
    prof = _tensorrt_dynamic_shape_profile_opts(model_name, onnx_path=onnx_path)
    if not prof:
...
class ModelsProcessor(QtCore.QObject):
    ...
    def load_model(self, model_name, session_options=None):
        ...
        with self.model_lock:
            ...
            model_trt_options = _merge_tensorrt_dynamic_shape_profiles(
                model_name, model_trt_options, onnx_path
            )
