...
class ModelsProcessor(QtCore.QObject):
    ...
    def _ensure_trt_ready_onnx(self, model_name: str, onnx_path: str) -> str:
        ...
        try:
            ...
            # Shape inference can take several seconds on large graphs (e.g. the
            # ~200 MB PerformRecast warping module). Surface a dialog so the UI
            # does not appear frozen during this one-time preprocessing step.
    ...
    def load_model(self, model_name, session_options=None):
        ...
        with self.model_lock:
            ...
            model_trt_options = merge_tensorrt_dynamic_shape_profiles(
                model_name, model_trt_options, onnx_path, control=_trt_ctrl
            )
