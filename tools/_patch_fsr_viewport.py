"""One-shot patch: FSR GL in viewport paintEvent. Run from repo root: python tools/_patch_fsr_viewport.py"""
from __future__ import annotations

path = r"app\ui\widgets\video_preview_fsr_gl_item.py"
...
helpers = """
    ...
    def _item_device_pixel_size_from_view(
        ...
        rw = max(1, int(__import__("math").ceil(max(xs) - min(xs))))
        rh = max(1, int(__import__("math").ceil(max(ys) - min(ys))))
        return rw, rh
...
# Read patch body from this file below marker
MARK = "# === RENDER_BODY_START ===\n"
idx = __doc__ or ""
# embed render body as separate string in file after marker - use open self
script = open(__file__, encoding="utf-8").read()
if MARK not in script:
    raise SystemExit("marker missing")
render_body = script.split(MARK, 1)[1].strip() + "\n\n"

i = data.find("    def paint(  # noqa: N802")
if i < 0:
    raise SystemExit("paint not found")
j = data.find("    def reset_gl_state(self)", i)
if j < 0:
    raise SystemExit("reset_gl_state not found")

data = data[:i] + helpers + render_body + data[j:]
...
# === RENDER_BODY_START ===
    def render_gl_in_viewport(
        ...
        try:
            ...
            self._tex_src = self._ensure_src_texture(w, h)
            if self._tex_src is None:
                self._gl_failed = True
                _fsr1_dbg("source texture allocation failed")
                return

            if self._present_seq != self._upload_applied_seq:
                rgb = _numpy_bgr_to_rgb_contiguous(self._frame_bgr)
                self._upload_tex(self._tex_src, rgb)
            ...
            if direct_blit:
                ...
                xf0 = ctx.extraFunctions()
                if xf0 is not None:
                    xf0.glActiveTexture(_GL_TEXTURE0)
                self._prog_blit.bind()
                self._tex_src.bind(0)
            ...
            if _fsr1_debug_env() and not getattr(self, "_fsr1_debug_logged_ok", False):
                ...
                _fsr1_dbg(
                    ...
                    f"glWidget={dev_cls} {vw}x{vh} tex={w}x{h} layout={self._lay_w}x{self._lay_h} "
