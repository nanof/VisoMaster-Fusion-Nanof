...
def bench():
    ...
    try:
        m4 = PeppaPig98Torch(ONNX_PATH, compute_dtype=torch.float16).cuda().eval()
    ...
    if not int(os.environ.get("PEPPAPIG_TORCH_COMPILE", "0")):
        print("  Skipped — reduce-overhead may crash on this model on Windows/sm_89.")
        print("  Set PEPPAPIG_TORCH_COMPILE=1 to attempt.")
    else:
        try:
            from custom_kernels.compile_utils import apply_torch_compile
            m4b = PeppaPig98Torch(ONNX_PATH, compute_dtype=torch.float16).cuda().eval()
    ...
    with torch.no_grad():
        pt_out = runner(inp)
    pt_lmk = pt_out[0].detach().cpu().numpy()  # (98, 3)
