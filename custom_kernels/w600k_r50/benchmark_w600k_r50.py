...
def bench_w600k_r50():
    ...
    if "TensorrtExecutionProvider" not in _ort.get_available_providers():
        ...
        if not ctx.exists():
            ...
            sess0b = _ort.InferenceSession(W600K_ONNX, so, providers=[
    ...
    m_fp32 = IResNet50Torch.from_onnx(W600K_ONNX, compute_dtype=torch.float32).cuda().eval()
    with torch.no_grad():
        t1 = _bench(lambda: m_fp32(inp_f32))
    _print_row("1", "PyTorch FP32 pure ops", t1, t0)

    # ── Tier 2 — PyTorch FP16 ─────────────────────────────────────────────
    m_fp16 = IResNet50Torch.from_onnx(W600K_ONNX, compute_dtype=torch.float16).cuda().eval()
    with torch.no_grad():
