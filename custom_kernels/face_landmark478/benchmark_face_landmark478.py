...
def bench():
    ...
    if not int(os.environ.get("LMK478_TORCH_COMPILE", "0")):
        print("  Skipped — reduce-overhead may crash on this model on Windows/sm_89.")
        print("  Set LMK478_TORCH_COMPILE=1 to attempt.")
    else:
        try:
    ...
    with torch.no_grad():
        pt_outs = runner(inp)
    pt_lmk = pt_outs[0].detach().cpu().numpy().reshape(478, 3)  # (478, 3) PyTorch
