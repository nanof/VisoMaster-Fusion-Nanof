...
class FaceBlendShapesTorch(nn.Module):
    ...
    def __init__(self, compute_dtype: torch.dtype = torch.float16):
        ...
        self.tok_up   = nn.ModuleList(
            [nn.Conv2d(self.N_TOKENS, self.TOKEN_HIDDEN,   1, bias=True) for _ in range(4)]
        )
        self.tok_down = nn.ModuleList(
            [nn.Conv2d(self.TOKEN_HIDDEN, self.N_TOKENS,   1, bias=True) for _ in range(4)]
        )

        # Channel mixing MLP — operates on (N, 64, 1, 97)
        self.ch_up    = nn.ModuleList(
            [nn.Conv2d(self.D_MODEL, self.CHANNEL_HIDDEN,  1, bias=True) for _ in range(4)]
        )
        self.ch_down  = nn.ModuleList(
            [nn.Conv2d(self.CHANNEL_HIDDEN, self.D_MODEL,  1, bias=True) for _ in range(4)]
        )
    ...
    def _ln(self, x: torch.Tensor, gamma: nn.Parameter) -> torch.Tensor:
        ...
        x_n = F.layer_norm(x_t, [self.D_MODEL],
                           weight=gamma, bias=None, eps=_LN_EPS)
    ...
    def from_onnx(
        ...
        for i, name in enumerate(_LN1_GAMMA_NAMES):
            m.ln1_gamma[i].data = torch.from_numpy(init_map[name].copy()).to(compute_dtype)
        for i, name in enumerate(_LN2_GAMMA_NAMES):
            m.ln2_gamma[i].data = torch.from_numpy(init_map[name].copy()).to(compute_dtype)

        # ── CLS token ────────────────────────────────────────────────────
        m.cls_token.data = torch.from_numpy(
            init_map[_CLS_TOKEN_NAME].copy()
        ).to(compute_dtype)

        print(f"[face_blendshapes] loaded: {ci} Conv + 8 LN-scale + 1 CLS weight tensors")
        return m
